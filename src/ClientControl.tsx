...
import { ISegmentServiceRow, TDataGridConfig, TTicket } from './types';
...
const ClientControl = React.memo(() => {
	const selectedTicket = React.useRef<TTicket | null>(null);
	...
	React.useEffect(() => {
		const [datagridRows, datagridColumns] = [server.getDatagridRows(), server.getDatagridColumns()];
		...
		({ employee }: { employee: TEmployee }) =>
			() => {
				const { tickets, columns } = server.updateTicket({
					employee,
					ticket: selectedTicket.current as TTicket,
				});
		...
		({ employee }: { employee: TEmployee }) =>
			() => {
				setModalConfig({
					...
					body: (
						<div className='confirm-transfer-ticket-container'>
							<div className='datagrid-row-ticket'>
								{selectedTicket.current!.ticketValue}
							</div>
							<Typography
								variant='primary'
					...
					modalActionsClassName: 'confirm-transfer-client-modal-actions-container',
	...
	const handleMapAvailableEmployesForSegment = React.useCallback((employee: TEmployee) => {
		const { id, position, employeeName, isActive, timeBeingActive, timeBeingInactive } = employee;
		return (
			<div key={`employee-${employeeName}`} className='segment-options-employee-container'>
				<Button
				...
				<div className='segment-options-employee-details'>
					...
					<Typography textClassName='segment-options-employee-name' variant='link'>
						{employeeName}
					</Typography>
				</div>
					...
					>
						{
							dateStringToMMSS({
								startDate: new Date(
									timeBeingActive || timeBeingInactive,
								),
							})?.parsed
						}
					</Typography>
	...
	const handleMapModalConfig = React.useCallback(
		(
			{ segmentName, numberOfTicketsInQueue, availableEmployeesForSegment }: ISegmentServiceRow,
			index: number,
			self: ISegmentServiceRow[],
		) => {
			return (
				<div key={`segment-${segmentName}`} className='segment-options-container'>
					<div className='segment-options-header'>
						...
						<Typography>
							...
							<span className='tickets-in-queue'>
					...
					<div className='segment-options-employees-container'>
						{availableEmployeesForSegment.map(handleMapAvailableEmployesForSegment)}
					</div>
				</div>
			);
		},
	...
	const handleRowClick = React.useCallback((row: TTicket) => {
